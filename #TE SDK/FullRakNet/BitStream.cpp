...
BitStream::BitStream( unsigned char* _data, unsigned int lengthInBytes, bool _copyData )
{
	...
	if (copyData) {
		if (lengthInBytes > 0) {
			...
			data = nullptr;  // Ensure data is set to nullptr if length is 0
		}
	}
	else {
...
{
	if (data != stackData && copyData && data != nullptr) {
		free(data);  // Free the previously allocated memory if any
	}
	data = input;
	copyData = false;
}
...
void BitStream::AddBitsAndReallocate(const int numberOfBitsToWrite)
{
	...
	{
#ifdef _DEBUG
		// Ensure that we can reallocate only if the data is copyable
		assert(copyData == true);
#endif

		// Double the memory allocation size for efficiency
		...
		// If we're currently using stack data and need more space
		if (data == (unsigned char*)stackData)
		{
			// Allocate memory dynamically only if the size exceeds stack allocation size
			if (amountToAllocate > BITSTREAM_STACK_ALLOCATION_SIZE)
			{
				// Allocate new memory
				unsigned char* newData = (unsigned char*)malloc(amountToAllocate);
				if (newData == nullptr) {
					// Handle memory allocation failure by logging or throwing an error
					// Optionally handle this case in a way suitable for your project
					return;
				}

				// Copy the stack data to the newly allocated heap memory
				memcpy(newData, stackData, BITS_TO_BYTES(numberOfBitsAllocated));
				data = newData; // Switch to using heap-allocated memory
			}
		}
		else
		{
			// If already on heap, try reallocating to get more memory
			unsigned char* newData = (unsigned char*)realloc(data, amountToAllocate);
			if (newData == nullptr) {
				// Handle memory allocation failure
				// Optionally log or return an error if realloc fails
				return;
			}

			data = newData; // Update to the new reallocated memory
		}

#ifdef _DEBUG
		// Ensure that the reallocation was successful
		assert(data);
#endif
}

	// Update the total allocated bits
